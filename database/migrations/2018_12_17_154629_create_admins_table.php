...
class CreateAdminsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('admins', function (Blueprint $table) {
            ...
            $table->string('show_name', 255)->comment('管理员真实名称');
            ...
            $table->integer('num')->default(0)->comment('登陆次数');
            $table->string('admin_use',4)->default('0')->comment('是否启用,1:开启,0:关闭');
            $table->integer('admin_group')->comment('所属分组');
            $table->tinyInteger('admin_method')->default('0')->comment('管理员读写权限 0：只读；1：读写');
            $table->tinyInteger('is_order')->default(0)->comment('是否查看订单完整信息，0：不查看 1：查看');
            $table->tinyInteger('admin_rule')->default(0)->comment('查看数据权限 :0：仅查看自己，1：查看自己与本组，2：查看全体成员，3：root');
